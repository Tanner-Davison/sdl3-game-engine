...
entt::entity SpawnPlayer(entt::registry&              reg,
    ...
    reg.emplace<Transform>(player, (float)(windowW / 2 - 33), (float)(windowH / 2 - 46));
...
void DoRetry(entt::registry&              reg,
             bool&                        gameOver,
             int                          windowW,
             int                          windowH,
             SDL_Surface*                 playerSheet,
             const std::vector<SDL_Rect>& walkFrames,
             SDL_Surface*                 enemySheet,
             const std::vector<SDL_Rect>& enemyFrames) {
    reg.clear();
...
int main(int argc, char** argv) {
    ...
    SDL_Rect retryBtnRect{
        GameWindow.GetWidth() / 2 - 75, GameWindow.GetHeight() / 2 + 10, 150, 55};
    ...
    SpawnPlayer(reg,
                ...
                playerSheet.GetSurface(),
                walkFrames);
    SpawnEnemies(reg,
                 GameWindow.GetWidth(),
                 GameWindow.GetHeight(),
                 enemySheet.GetSurface(),
                 enemyWalkFrames);
